...
- `crontab-redacted.mjs` — the sanctioned way to READ a crontab (V-594). `crontab -l` prints the crontab verbatim and a crontab routinely carries a credential inline (env line, `--password` on a job, URL userinfo); that leaked a live password into a transcript, so `guard-sensitive-access.py` now DENIES `crontab -l` and allowlists this verb instead — the same "allow the verb, not the source" shape as `transcript-resolver.mjs`. Redaction is deliberately broader than that module's `redact()`: **every** `NAME=value` value is masked whatever the name (the key-name heuristic is exactly what missed V-594), plus secret-looking flag values and URL userinfo, with `redact()` as a second pass. Schedule fields, command names and comments survive. `[-u <user>]`; exits with crontab's own status. Auto-allowlisted by `Bash(node ~/.claude/bin/*.mjs)`.
...
- `custom-model-metadata.mjs` — validate & resolve the routed custom-model registry `models/custom-models.json` (V-383). `validate` enforces the invariants JSON Schema can't (provenance is mandatory; provider docs can't satisfy a route's live gate; `preferredContextMode: "extended"` requires a route `verified` past the 200K gateway floor with matching evidence). `resolve <id>` is a pure map from a requested id to the expected wire id (`[1m]` stripped) + the plain fallback — it never launches Claude or edits settings. Stdlib only; auto-allowlisted by `Bash(node ~/.claude/bin/*.mjs)`. Exit `0` OK · `1` invalid/gate-not-met · `3` bad args.
- `probe-custom-model-context.mjs` — the two empirical probes behind the registry's evidence (V-383). `capture` proves Claude Code strips `[1m]` before the id leaves the client: it stands up a disposable loopback Anthropic-API server, points ONLY the child Claude at it via `--bare --settings` with a dummy key (no provider contacted, no credential read), and records the wire `model`. `live` proves the configured route accepts a request past 200K: a deterministic filler prompt through the normal router, sentinel reply, returned usage read back — fail-closed if the route is down, truncates, or reports ≤200K. Output is sanitized JSON (ids, counts, versions, generator hash) — never prompt/response bodies, headers, or secrets. It never edits the registry; a human reviews the evidence. Exit `0` pass · `1` fail · `2` unavailable · `3` bad args.
