#!/usr/bin/env bash
# install-daily-summary-launchd.sh — install (idempotent) the evening /daily-summary run as a
#   macOS launchd LaunchAgent. (V-263 — twin of install-feedback-harvest-launchd.sh)
#
...
#   cannot back a durable unattended ritual. A cloud /schedule routine can't read the LOCAL, gitignored
#   data the summary consumes (pipeline/audit/gate-audit.md, feedback.jsonl, .claude/usage-stats/*.json)
#   — only a process on THIS machine can. So the durable-AND-local path is launchd, like the harvesters.
#
# SCHEDULE: daily 20:37 local (evening) — an odd minute to avoid coinciding with any :00/:30 fleet job.
#
...
#   launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.v-coding-setup.daily-summary.plist
#   && rm that plist.

set -euo pipefail

LABEL="com.v-coding-setup.daily-summary"
PLIST="$HOME/Library/LaunchAgents/$LABEL.plist"
RUNNER="$HOME/.claude/bin/daily-summary-runner.sh"
LOG="$HOME/.claude/pipeline/audit/daily-summary.log"
GUI="gui/$(id -u)"
...
# 1. Write the LaunchAgent plist (daily 20:37 local). Paths are baked in absolute at install time.
cat > "$OUT" <<PLIST_EOF
...
echo "Installed launchd daily-summary agent: $LABEL"
echo "  plist  -> $PLIST"
echo "  runner -> $RUNNER"
echo "  log    -> $LOG"
echo "  schedule: daily 20:37 local (a slot missed while asleep runs on next wake)"
