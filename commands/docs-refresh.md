...
- **`--yes` (daily cron / headless) — apply + PR, never merge:**
  ...
  - **Advance the watermark** only on a real (non-dry-run) pass, via the fs-write helper: `node ~/.claude/bin/advance-docs-refresh-watermark.mjs --commit $END_SHA` (the §1-pinned window end — never a freshly re-resolved `origin/<baseBranch>`, which could have moved and would skip the in-between merges forever; never a `Write`/`>` — that trips the sensitive-file prompt and freezes the unattended cron; mirrors `advance-feedback-watermark.mjs`).
