...
- **The aggregate — `node ~/.claude/bin/scorecard.mjs --aggregate --json`** (allow-listed via `Bash(node ~/.claude/bin/*.mjs)`). This single call is the reader of **usage-stats** (`costTickets` for compatible output-token ranking, `billingTickets` + `accounting` for model/billing classes, `costTools` for re-read bytes), **session reports** (`reviewAttribution`), **tool-fit** (`ceremonySteps`), **produced-code** (`qualOffenders`), and **gate friction** (`gateRows`, `pruneCandidates`, `loadBearing`), plus `recommendations` and `_sinks`. Consume its JSON; do not re-derive any of it.
- **Delivery — `node ~/.claude/bin/usage-report.mjs --since <window-start> --until <window-end> --json`.** This helper reads all usage-stats before filtering and allocates cumulative snapshots by session/assistant ordinal, so multiple lands in one session do not double-count. Its `lands[]` are the delivery set with ticket, PR, deduped token totals, per-model usage, and the four accounting classes. Cross-reference `produced-review.jsonl` for reviewed lands. Optional Linear enrichment: `get_issue` per in-window ticket only.
...
- Run `usage-report.mjs --since <start> --until <end> --json`; use its `lands`, `totals`, and `accounting` verbatim. Join each land to `qualOffenders` / `produced-review.jsonl` where a produced-review exists.
...
- **Cost & tool-fit trends** — use `billingTickets`/`accounting` for priced model-aware comparisons, `costTickets` only as the compatible output-token ranking, and `costTools` for re-read bytes. Keep actual API, API-equivalent, subscription-no-bill, and unknown/unpriced figures separate; never call their sum spend.
...
- **Delivery review (tickets completed using V)** — per in-window allocation: what shipped (ticket + PR), its produced-review verdict, deduped tokens, models, and applicable accounting class. Compare estimates only within the same class; subscription usage has no token bill, and unknown/unpriced usage is a coverage gap rather than $0.
