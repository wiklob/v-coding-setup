...
2. **Delivery dimension.** `/scorecard --aggregate` is pipeline-meta only. This also reviews **what shipped using V** in the window — the tickets completed, enumerated from the global `produced-review.jsonl`/`tool-fit.jsonl` land ledger (V-447) — so the review covers both the pipeline's self-improvement AND the project work it produced.
...
- **Delivery — the land ledger, joined from the two global per-land review sinks (V-447; NOT enumerated from `usage-stats/`).** `pipeline/audit/produced-review.jsonl` (`ticket`, `pr`, `ts`, `quality`) and `pipeline/audit/tool-fit.jsonl` (`ticket`, `ts`, `steps`) are both appended **unconditionally, by every `/land-ticket` §8.6 run, in ANY repo** — that's what makes their union the correct "tickets completed using V" set. Union the `ticket`s from both sinks whose `ts ≥` the window start; that set IS the delivery set. (`usage-stats/` is a per-land token-cost lookup, not the land enumeration — see below; a land whose §8.5 stats write failed still shows up here via the review sinks, cost reported as unavailable rather than the land vanishing.) For each land's token cost, look up `~/.claude/.claude/usage-stats/<...>-<TICKET>.json` by its content `ticket` field: as of V-681 `usage-stats.mjs` resolves that sink from its own install location, not the caller's cwd, so every repo's land writes into the same global directory `scorecard.mjs`'s `costTickets` already reads — no per-repo lookup or index required. Optional Linear enrichment: `get_issue` per in-window ticket for its title/state — keep it to the in-window set, never a broad `list_issues`.
...
- Build the delivery set from `produced-review.jsonl` ∪ `tool-fit.jsonl` (union of `ticket`s with `ts ≥` window start) — **not** by listing `usage-stats/` (V-447: that directory is a global per-land cost lookup, never the land enumeration). For each ticket in the set: pull `pr` + `quality` from its `produced-review.jsonl` row when present; pull its token totals from `usage-stats/*.json` by content `ticket` field (now global per V-681 — every repo's land resolves there); a ticket with no matching `usage-stats` file still appears, cost noted "unavailable" rather than dropped.
