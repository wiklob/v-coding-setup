...
3. Append exactly one entry by invoking the feedback writer in its **absolute main-checkout path** (so the entry always lands in the global `~/.claude/pipeline/audit/feedback.jsonl`, regardless of the session's cwd):
   ...
   - **Report the incident verbatim — describing an act is not performing it (V-593).** A note may quote the exact command that misbehaved (`cat .envrc`, `printenv`, `env | grep TOKEN`): `guard-sensitive-access.py` recognizes a single, non-substituting logger call as an inert write and does not scan its quoted text. Never soften, paraphrase or elide a note to get past a guard — a leak report that omits the command is evidence lost. Two constraints keep that exemption honest, so keep the call **one self-contained command**: nothing chained, piped or redirected onto it (convention 7 wants a discrete call anyway), and no `$(…)` / backticks / `<(…)` in the note text — those execute while the argument is formed, so the guard scans them in full and will, correctly, deny.
