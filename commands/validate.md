...
- Resolve the target, in order: the explicit `$ARGUMENTS` `[PROJECT]` (name or id) → a helper-backed binding on a linked `$WT_ABS` → otherwise STOP and ask (`needs input: /validate needs a project — pass it as the arg.`). For the fallback, compare the checkout's absolute `--git-dir` and `--git-common-dir`; only when linked, run `node ~/.claude/bin/ticket-worktree.mjs migrate-binding --worktree "$WT_ABS"` before presence/read, then `node ~/.claude/bin/ticket-worktree.mjs binding-status --worktree "$WT_ABS"` and (when `private`) `node ~/.claude/bin/ticket-worktree.mjs read-binding --worktree "$WT_ABS"`. Conflicts/malformed state hard-STOP; `none` means no inferable project. A standalone binding (`linearIssue`, no `linearProject`) is a single-ticket worktree, not a project to validate → take the issue's parent project (one `get_issue`) or ask. Never access the legacy checkout marker directly.
