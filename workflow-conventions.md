...
- **The exact installed `~/.claude` source remains the compatibility exception until V-376.** Its ticket worktrees stay siblings, avoiding the nested `~/.claude/.claude/worktrees/...` path. Do not generalize this exception to ordinary repositories or to any path that merely contains `.claude`.
