...
- **An env-needing probe (a throwaway script hitting a live service) runs through `bin/wt-env` (V-523), never `eval "$(direnv export bash)"`.** That compound is refused by the worktree-isolation guard as "too complex to verify". `wt-env <cmd> [args…]` sources `./.envrc` and execs the command as one plain, guard-safe call; when `<cmd>` is `python`/`python3` and the repo has `./.venv/bin/python`, `wt-env` resolves to that interpreter automatically (falling back to PATH otherwise), so `wt-env python3 <script>` carries both the repo's deps and its direnv env in a single invocation — no separate venv-selection step needed.
- **Scratch & audit writes in an isolated background session stay inside the active worktree, or go through a sanctioned helper.** This rule is layout-independent: `$WT_ABS/tmp/` is the throwaway scratch root for both managed worktrees and a temporarily retained legacy sibling; an append that must survive teardown in the canonical checkout goes through a reviewable `bin/*.mjs` helper, never an Edit/Write call into the shared checkout and never a raw redirection. The generic non-worktree scratch root below does not override this once the session is worktree-isolated.
  - **No worktree yet ⇒ scratch lives under the system temp root, never under `~/.claude` (V-441).** `$CLAUDE_JOB_DIR` resolves under `~/.claude`, which the harness treats as a protected path — a plain `mkdir -p "$CLAUDE_JOB_DIR/tmp"` raises a sensitive-file permission prompt with no one to answer it in a background session (allow-listing does not help; `.claude` is protected against allow-rule override, claude-code#66525). Use `${TMPDIR:-/tmp}/claude-scratch/$(basename "$CLAUDE_JOB_DIR")` instead — a per-job directory outside `~/.claude`, prompt-free to `mkdir -p`. This is the "otherwise" branch whenever a bg session has no worktree yet (e.g. `/go` halting at Phase 1 before `$WT_ABS` is set).
  - **The model's own auto-memory notes are a special case of this, not an exception (V-365).** `~/.claude/projects/<encoded-cwd>/memory/<slug>.md` lives outside every worktree — there is no worktree to re-enter for it, post-teardown or otherwise — so an Edit/Write call there is refused with no working alternative unless routed through a helper too: `<note prose> | node ~/.claude/bin/write-memory-note.mjs --path ~/.claude/projects/<encoded-cwd>/memory/<slug>.md`. It validates the target stays inside the home directory and under a `memory/` dir before writing, so a worth-keeping lesson learned mid-isolation is no longer silently dropped.
  - **Spawned subagents need an absolute scratch path in their dispatch prompt (V-322).** Resolve and embed the literal `$WT_ABS/tmp/` when isolated, otherwise the literal `${TMPDIR:-/tmp}/claude-scratch/$(basename "$CLAUDE_JOB_DIR")`; never `/tmp` via a deep relative path and never `$CLAUDE_JOB_DIR/tmp`.
...
- **An allow rule is necessary but not sufficient — the harness's auto-mode classifier is a separate layer, and it can still refuse an allow-listed or sanctioned command.** `Blocked by classifier` names no rule; it is neither a `permissions.deny`/`ask` decision (different message shape) nor a `bin/*` guard (those print their own reason). Observed evidence: a command matching a settings.json `permissions.allow` prefix rule — including a single-token invocation of a sanctioned helper (`wt-env`, a literal `docker stop <id>`) — has still been refused this way, and a verbatim retry of the identical command failed identically. Whether the classifier is consulted before or after `permissions.allow` internally is not something this repo's config can establish (the classifier is harness-internal); treat matching an allow rule as improving the odds, not guaranteeing a pass. When it fires: never retry the same command — split or simplify it (drop a pipe, un-chain a compound) and reissue; after repeated consecutive blocks on the same shape the harness escalates to a user-confirmation prompt, which is the correct off-ramp, not something to route around.
...
**Dedupe before filing.** Before `save_issue`/`create_issue`, `list_issues` the target bucket/project for an existing open ticket describing the same defect (same file:line or same symptom in the title/body) and skip filing if one is already live — same discipline the verify-the-fix/check-later spin-outs below apply via their `monitor-key`/`defer-key`, extended to this plainer triage case where there's no single dedupe-key line to match on, so match on symptom/location instead. A repeat run of the same build/manual-test pass must not mint a second ticket for a defect it already triaged once.
